...
*/
extern volatile uint8_t I2CSlaveState;
extern volatile uint8_t I2CMode;
extern volatile uint8_t I2CWrBuffer[BUFSIZE];
extern volatile uint8_t I2CRdBuffer[BUFSIZE];
extern volatile uint8_t RdIndex ;
extern volatile uint8_t WrIndex ;

volatile uint8_t I2C_Data_Adress;	
volatile uint8_t I2C_St_Adress;	

extern volatile U32_T bright_level_fan;
extern volatile U32_T bright_level_light;
...
{
	if(bright_level_light == bright_level_fan){						//如果风扇与灯亮度相等
		tim1_int_cb();
		tim2_int_cb();  
		...
		if(Flag==0){											
			if(bright_level_light > bright_level_fan){				//灯亮度大于风扇亮度
				tim1_int_cb();
			}else
			{
				tim2_int_cb();
